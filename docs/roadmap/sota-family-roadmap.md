# RETIRED — family roadmap pointer

This file previously implied a multi-product family control plane.

**Retired.** Product strategy and delivery live in this repository only.

Use:

- [POSITIONING.md](../POSITIONING.md)
- [COMPETITIVE.md](../COMPETITIVE.md)
- [IPPB.md](../IPPB.md)
- [PRODUCT_INDEPENDENCE.md](../PRODUCT_INDEPENDENCE.md)

Historical multi-repo “family SSOT” repos (`reader-evidence`, `products`) are archived/non-authority.
